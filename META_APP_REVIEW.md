...
`instagram_business_manage_insights`. We read the connected account's own follower count once a day to show the account owner a follower-growth chart alongside their campaign results. We do not read insights for any account other than the one that authorized us, and we do not read audience demographics.
...
3. Create a campaign on a recent post with keyword `LOVE15`, a DM message with a link, and save.
4. On a second phone or account, comment `LOVE15` on that post.
5. Show the second account receiving the DM, and the public reply appearing under the comment.
6. Back in the app, show the DM Logs page with the SENT row, and the Overview page with the follower chart (covers `instagram_business_manage_insights`).
